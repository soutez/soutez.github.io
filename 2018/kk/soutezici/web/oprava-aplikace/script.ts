namespace StuffSimulator {

	type PositionData = [number, number]
	/** Reprezentuje jeden vrchol mapy (křižovatku) */
	type NodeData = {
		/** pozice na ploše */
		pos: PositionData
	}
	/** Propojení dvou bodů na mapě */
	type NodeConnectionData = {
		/** z jaké Node to vede, cesta je nicméně obousměrná */
		from: number
		/** do jaké Node to vede */
		to: number
		/** Křivka cesty - [0,0] je na začátku */
		curve: PositionData[]
		/** Jak dlouho trvá tudy projet (v simulovaných sekundách) */
		distance: number
	}
	type StationData = {
		/** číslo Node, kde se stanice nachází */
		node: number
		/** jméno */
		name: string
	}
	...
	/** Jeden záznam v jízním řádu */
	type TimetableRow = {
		/** Směr jízdy */
		direction: -1 | 1,
		/** Z jaké stanice (index v seznamu stanic linky) spoj jede (defaultě je to konečná určená podle směru) */
		from?: number,
		/** Kdy vyjíždí */
		time: number
	}
	...
	type LineDefinition = {
		name: string
		/** Jakými stanicemi vede (čísla stanic) */
		stops: { id: number, time?: number }[]
		/** Jízdní řád */
		timeTable: TimetableRow[]
	}
	type SimulationAction = "continueLine" | "die" | { type: "continuePath", nextConnection: number, followingAction: SimulationAction }
	/** Reprezentuje objekt simulace */
	type SimulationObject = {
		/** Na jaké lince, v jakém směru a na jaké pozici se objekt vyskytuje */
		line?: { direction: -1 | 1, id: number, position: number }
		/** Na jakém propojení se vyskytuje */
		path?: number
		/** Odkud jede */
		startNode: number
		/** Kdy se začal pohybovat */
		startTime: number
		/** Kdy s akcí končí */
		endTime: number
		/** Co se stane až skončí */
		followingAction: SimulationAction
	}
	type SimulationEvent = {
		time: number
		spawnedObject: SimulationObject
	}
	type SimulationData = {
		map: MapData
		...
		currentTime: number
		/** rychlost běhu v jednotkách za sekundu */
		speed: number
		/** Seznam žijících objektů */
		livingObjects: SimulationObject[]
		/** O kolik se liší interní čas a čas zobrazený uživateli */
		displayTimeOffset: number
	}
	type SimulationRenderData = {
		simulation: SimulationData
		/** Jestli simulace běží */
		running: boolean
	}

	type Renderer<TData, TElement> = (data: TData, element: TElement) => void

	const parseMapFromFile = (file: File) =>
		new Promise<SimulationData>((resolve, reject) => {
	...
	const initializeSimulation = (sim: Partial<SimulationData>): SimulationData => {
	...
	const initializeRenderData = (simulation: SimulationData) => {
	...
	class PriorityQueue<T> {
		private data: T[] = []
		constructor(private getKey: (a: T) => number) { }
		...
		public push(...items: T[]) {
			...
			if (!this.data.map(this.getKey.bind(this)).every(m => m >= min)) fail()
		}

		public peek() { return this.data[this.data.length - 1]; }
		public pop() { return this.data.pop(); }
		public empty() { return this.data.length == 0; }
	}

	const fail = (message = "Assertion failure"): never => { throw new Error(message) }
	const cachedFunction = <T extends object, U>(func: (arg: T) => U) => {
	...
	const getConnectionMap = cachedFunction((map: MapData) => {
		const result: { conn: NodeConnectionData, connId: number, to: number }[][] = map.nodes.map(n => [])
	...
	const findPath = (map: MapData, from: number, to: number): { node: number, connId: number }[] => {
		if (from == to) return [];
		const connectionMap = getConnectionMap(map)
		const q = new PriorityQueue<{ node: number, dist: number }>(a => a.dist)
		q.push({ node: to, dist: 0 })
		const pathMap: { [nodeId: number]: { node: number, connId: number, dist: number } } = {}
		...
		if (!(from in pathMap)) fail(`Could not find path from node #${from} to node #${to}.`)
	...
	const createEventForStation = cachedFunction((map: MapData) => cachedFunction((fromStation: StationData) => cachedFunction((toStation: StationData) => {
		const path = findPath(map, fromStation.node, toStation.node)
		var action2: SimulationAction = "continueLine"
		...
		return (startTime: number, line?: { direction: -1 | 1, id: number, position: number }): SimulationEvent => {
			return {
				time: startTime,
				spawnedObject: {
					line,
	...
	const getGlobalEventList = cachedFunction((map: MapData) => cachedFunction((lines: ReadonlyArray<LineDefinition>) => {
	...
	const moveSimulation = (data: SimulationData, deltaTime: number): SimulationData => {
		...
		const processObject = (obj: SimulationObject) => {
			if (obj.followingAction == "die") {
			} else if (obj.followingAction == "continueLine") {
				const oldLine = obj.line || fail()
				const line = { ...oldLine, position: oldLine.position + oldLine.direction }
		...
		return {
			...
			map: data.map,
			currentTime: time,
			livingObjects: triggeredObjects.length == 0 && newObjects.length == 0 ? data.livingObjects : data.livingObjects.filter(o => o.endTime > time).concat(newObjects),
		}
	}

	const getObjectProgress = (obj: SimulationObject, time: number) => {
		if (time <= obj.startTime)
			return 0;
		else if (time >= obj.endTime)
			return 1
		else return (time - obj.startTime) / (obj.endTime - obj.startTime)
	}

	// rendering
	const createMapRenderer = () => {
		const svgNS = "http://www.w3.org/2000/svg"
		const svgAttr = (element: SVGElement, name: string, value: string) =>
			element.setAttributeNS(<any>null, name, value)

		const updateCollection = <T, TElement extends Element>(
			elements: TElement[],
			rootElement: Element,
			newData: T[],
			oldData: T[] | null | undefined,
			updateFn: Renderer<T, TElement>,
			factoryFn: () => TElement,
			alwaysUpdate = false
		) => {
			if (newData == oldData && !alwaysUpdate) return;
			if (elements.length == newData.length) {
				...
				for (const p of elements) p.remove()
		...
		var paths: SVGPathElement[] = [],
			stations: SVGPathElement[] = [],
			objects: SVGPathElement[] = []

		const mapRenderer = (() => {
			var lastData: MapData | undefined
			return (data: MapData, element: SVGElement) => {
				if (lastData == data) { return }
				if (lastData == null) {
					Array.from(element.childNodes).forEach(n => element.removeChild(n))
				}

				const renderPath = (conn: NodeConnectionData, element: SVGPathElement) => {
				...
				const renderStation = (station: StationData, element: SVGPathElement) => {
					...
					if (!title.parentNode) element.appendChild(title)
				}
		...
		var lastData: SimulationRenderData
		return (data: SimulationRenderData, element: SVGElement) => {
			if (lastData === data) return

			mapRenderer(data.simulation.map, element)
			const time = data.simulation.currentTime

			const renderObject = (obj: SimulationObject, element: SVGPathElement) => {
				const position = (() => {
					if (obj.path != null) {
						const progress = getObjectProgress(obj, time)
						const path = data.simulation.map.connections[obj.path]
						if (path.from != obj.startNode && path.to != obj.startNode) fail()
					...
					} else return data.simulation.map.nodes[obj.startNode].pos
				})()
	...
	const saveMapToLocalStorage = (map: Partial<SimulationData>) => {
		try {
	...
	const tryLoadMapFromLocalStorage: () => (Partial<SimulationData> | null) = () => {
		try {
	...
	export class ControlPanel {
		constructor(
			...
			playButton.onclick = () => this.updates.push(state => ({ ...state, running: true }))
			pauseButton.onclick = () => this.updates.push(state => ({ ...state, running: false }))
			resetButton.onclick = () => this.updates.push(state => {
				const simulation = { ...state.simulation, currentTime: 0, livingObjects: [] }
				return { ...state, simulation }
			})
			const speedOnChange = () => this.updates.push(state => {
				const speed = Math.pow(2, parseFloat(speedInput.value))
				const simulation = { ...state.simulation, speed }
				return { ...state, simulation }
			})
		...
		private updates: ((state: SimulationRenderData) => SimulationRenderData)[] = []

		public updateState(state: SimulationRenderData) {
			for (const update of this.updates) {
				state = update(state)
			}
			if (this.updates.length) this.updates = []
		...
		static create(someElement: HTMLElement): ControlPanel {
			return new ControlPanel(
				someElement.querySelector("#playButton") || fail(),
				someElement.querySelector("#pauseButton") || fail(),
				someElement.querySelector("#resetButton") || fail(),
				someElement.querySelector("#speedRange") || fail(),
				someElement.querySelector("#timeDisplay") || fail(),
			)
		}
	}

	export class App {
		private data?: SimulationRenderData
		private mapRenderer?: Renderer<SimulationRenderData, SVGElement>
		constructor(
		...
		loadMapFromFile(file: File) {
			return parseMapFromFile(file)
				.then(map => this.loadNewMap(map),
					error => alert("Error loading the file: " + error))
		}
		...
		loadNewMap(data: Partial<SimulationData>) {
		...
		saveMap(link: HTMLAnchorElement): boolean {
			if (this.data == null) return false
			const data = JSON.stringify({ ... (this.data.simulation), livingObjects: undefined, currentTime: undefined }, null, "\t")
		...
		importEpicMap() {
			const normalizeName = (stName: string) => stName.replace(/\(.*\)/, "").trim().toLowerCase()

			const stops: { name: string, midPoint: PositionData }[] = JSON.parse(prompt("paste the stops") || fail())
			const lines: { [lineName: string]: { timetable: { time: number, stop: string }[], departures: number[] }[] } = JSON.parse(prompt("paste the lines") || fail())
			...
			const nodes: NodeData[] = activeStops.map(s => ({ pos: <[number, number]>[s.midPoint[0] * 100, -s.midPoint[1] * 100] }))
			const stations: StationData[] = activeStops.map((s, index) => ({ name: s.name.trim(), node: index }))
			const stopNameMap = new Map(activeStops.map((s, index) => <[string, number]>[normalizeName(s.name), index]))
			const getStation = (st: string) => {
			...
			const dedupeLine = <T, TKey>(array: T[], key: (l: T) => TKey) => {
				const result = []
				var lastKey: TKey | undefined = undefined
				for (const i of array) {
					const k = key(i)
					if (k != lastKey) result.push(i)
					lastKey = k
				}
				return result
			}

			const resultLines: LineDefinition[] = []
			const resultConnections: NodeConnectionData[] = []
			const connectionDedupSet: Set<string> = new Set()
			for (const lineName of Object.keys(lines)) {
				const lol = lines[lineName]
				const stations: number[] = []
				const departures: TimetableRow[] = []
				for (const table of lol) {
					...
					if (commonPoint) {
						const direction: 1 | -1 = stations[commonPoint[1] - 1] == (timetable[commonPoint[0] - 1] && getStation(timetable[commonPoint[0] - 1].stop)) ? 1 : -1
						...
						departures.push(...table.departures.map(time => ({ from: 0, direction: <1>1, time: (time + 20 * 60) % (24 * 60) })))
					}
			...
			const map: MapData = { nodes: nodes, connections: resultConnections, stations: stations }

			const simData: SimulationData = { map, lines: resultLines, livingObjects: [], currentTime: 0, speed: 1, displayTimeOffset: 4 * 60 }

			this.loadNewMap(simData)
		}

		private renderDispatched = false
		private lastAnimationTime?: number
		private renderFrameId = -1
		dispatchRender() {
			if (this.renderDispatched) { return }
			...
			this.renderFrameId = window.requestAnimationFrame(time => {
				...
				if (this.lastAnimationTime != null && this.data.running) {
					...
					this.data = { ...this.data, simulation: moveSimulation(this.data.simulation, deltaTime / 1000) }
				}
